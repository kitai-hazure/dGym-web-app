...
import "./pose-estimator.component.css";
...
export default class PoseEstimator extends React.Component<any, any> {
  ...
  componentDidMount() {
    this.overlayPoseVisualizer = new PoseVisualizer({
      canvas: this.overlayCanvas.current,
    });
    this.previewPoseVisualizer = new PoseVisualizer({
      canvas: this.previewCanvas.current,
    });
    ...
    this.poseEstimatorService.onPoseEstimation(
      this.processPoseEstimation.bind(this)
    );
    this.poseEstimatorService.onActionEstimation(
      this.processActionEstimation.bind(this)
    );
  }
  ...
  async processActionEstimation(data) {
    ...
    if (data?.action && data?.score >= this.getProp("minScoreToDraw")) {
      ...
      data?.score >= 0.5 &&
      this.getProp("estimatedAction") !== "UNKONWN"
    ) {
  ...
  onCanvasHover(hoverEvent) {
    ...
    const checkIfInPointBoundingBox = ({ mouseLocation, pointLocation }) => {
      return (
        Math.abs(
          mouseLocation.x - this.distanceThreshold.x - pointLocation.x
        ) <= 5 &&
        Math.abs(
          mouseLocation.y - this.distanceThreshold.y - pointLocation.y
        ) <= 5
      );
    };
  ...
  drawPose() {
    this.overlayPoseVisualizer.loadPose({
      keypoints: this.pose.keypoints,
      score: this.pose.score,
    });
    ...
    this.overlayPoseVisualizer.drawOverlayOnCanvas({
      minScoreToDraw: this.getProp("minScoreToDraw"),
      autoMinScore: this.getProp("autoMinScore"),
      transparency: null,
    });
  ...
  async loadVideo(stream = null) {
    ...
    const drawToCanvasLoop = async () => {
      const isVideoPlaying =
        !this.videoPlayer.current.paused && !this.videoPlayer.current.ended;
      if (!isVideoPlaying) {
        return;
      }
      if (
        this.previewCanvas.current.width === 0 ||
        this.previewCanvas.current.height === 0
      ) {
        this.resizeCanvas({
          width: this.videoPlayer.current.videoWidth,
          height: this.videoPlayer.current.videoHeight,
        });
      }
      // this.previewPoseVisualizer.drawByMemo(this.videoPlayer.current, 200, 200);
      this.poseEstimatorService.loadImageToCanvas(
        this.videoPlayer.current,
        true
      );
      if (isVideoPlaying) {
        this.interval = setTimeout(async () => {
          await this.poseEstimatorService.loadImageAndRunPosenet(
            this.videoPlayer.current,
            true
          );
    ...
    this.videoPlayer.current.addEventListener(
      "play",
      drawToCanvasLoop.bind(this)
    );
    this.videoPlayer.current.addEventListener("stop", () =>
        ...
        <div className="pose-display">
          <div className="pose-info">
            <video
              ...
              style={{
                display: this.getProp("videoSelected") ? "initial" : "none",
              }}
            ...
            <div className="preview-container">
              ...
              <div className="canvas-container">
                ...
                <canvas
                  ...
                  {...PoseEstimator.DIMENSIONS}
                  onMouseDown={this.onCanvasHover.bind(this)}
                ></canvas>
                <canvas
                  ref={this.poseOnlyCanvas}
                  {...PoseEstimator.DIMENSIONS}
                ></canvas>
              </div>
            </div>
            ...
            <div>
              ...
              {/* <FormControlLabel
                ...
                control={
                  <Checkbox
                    onChange={(e) =>
                      this.setProp("autoMinScore", e.target.checked)
                    }
              ...
              <FormControlLabel
                ...
                control={
                  <Checkbox
                    onChange={(e) =>
                      this.setProp("renderLines", e.target.checked)
                    }
              ...
              <FormControlLabel
                ...
                control={
                  <Checkbox
                    onChange={(e) =>
                      this.setProp("showDetailedScore", e.target.checked)
                    }
